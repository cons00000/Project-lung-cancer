            ...
            for patient in tqdm(self.__patients):
                    ...
                    for root, dirs, files in os.walk(os.path.join(self._data_path)):
                        for file in files:
                            if file.endswith('.dcm'):
                                temp_file = pydicom.dcmread(os.path.join(root, file),force = True)
                ...
                if messy_structure:
                    for rt in all_dicom_subfiles:
                        temp_rt = pydicom.dcmread(rt, force=True)
                        for dfile in dcm_files:
                            temp_dfile = pydicom.dcmread(dfile, force=True)
                    ...
                    for file in dcm_files:
                        structfile = ''
                        try:
                            temp_file = pydicom.dcmread(file, force=True)
